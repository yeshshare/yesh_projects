...
@section('pluginsjs')
  ...
  <script>
    ...
    new gridjs.Grid({
      ...
      columns: ['Id','Company','Name','Email','Img','Actions' ],
		  search: true,
      data: users.data.map(user => [
            ...
            user.name, 
            user.email,
            gridjs.html(`<img src="data:image/jpeg;base64,${user.img}"></img>`),
            gridjs.html(
             `<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                <div class="btn-group" role="group">
                  <button id="btnGroupDrop${user.id}" type="button" class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                  </button>
                  <ul class="dropdown-menu" aria-labelledby="btnGroupDrop${user.id}">
                    <li><a class="dropdown-item edit" href="#"  data-bs-toggle="modal" data-bs-target="#form_modal" onclick="form_action_edit('${user.id}')" >Editar</a></li>
                    <li>
                      <a class="dropdown-item" method="DELETE" href="" data-bs-toggle="modal" data-bs-target="#form_modal" onclick="form_action_delete('${user.id}')">Excluir</a>
                    </li>                                
                  </ul>
                </div>
              </div>
...
@section('content')
    <div class="container">
		<div class="row">
			<div class="col-10">
			  <h2 class="main-title">{{ __('User') }}</h2>
			</div>
			<div class="col">
			  <div class="float-right" >
				<!--<a href="{{ route('users.create') }}" class="btn primary-default-btn btn-primary btn-sm float-right"  
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">
            <div class="col2">
                <form id="form"  method="POST" action="{{ route('users.store') }}"  role="form" enctype="multipart/form-data">
                    @csrf    
